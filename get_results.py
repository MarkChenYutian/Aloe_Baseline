...
def main(unused_argv):
    ...
    for sample_scene_idx in range(NUM_SCENES):
        ...
        num_questions = scene_json["questions"][-1]["question_id"] + 1
        
        scene_monet_latents = load_monet_latents(base_dir, sample_scene_idx + 10000)

        for question_id in range(num_questions):
            question_json = scene_json["questions"][question_id]

            question_result = dict()
            question_result["sceneid"] = str(sample_scene_idx)
            question_result["qid"] = str(question_id)
            question_result["question"] = question_json["question"]
            question_result["question_type"] = question_json["question_type"]

            if question_json["question_type"] == "descriptive":
                question_result["question_subtype"] = question_json["question_subtype"]
                question_result["choices"] = None
                question_result["gt"] = question_json["answer"]
                ans = eval_descriptive(scene_monet_latents, question_json)
                question_result["answer"] = ans
                
            else:
                question_result["question_subtype"] = None
                question_result["choices"] = [choice_json["choice"] for choice_json in question_json["choices"]]
                question_result["gt"] = [choice_json["answer"] for choice_json in question_json["choices"]]
                anss = eval_mc(scene_monet_latents, question_json)[0]
                res = []
                for choice_id, choice_json in enumerate(question_json["choices"]):
                    ans = pred_map[anss[choice_id]]
                    res.append(ans)
                question_result["answer"] = res

            results.append(question_result)

        progress_bar.update()

    with open("{}_{}_formatted_results.json".format(START, START + NUM_SCENES), "w+") as file:
